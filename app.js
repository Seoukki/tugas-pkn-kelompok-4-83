...
const Arena=(()=>{
  ...
  function drawFlag(fx){
    const py=H*.5,ph=H*.27;
    // Static flag pole - no wave animation
    ...
    // Static flag - no wave
    cx.fillStyle='#DC2626';cx.beginPath();cx.moveTo(0,0);cx.quadraticCurveTo(fw*.5,0,fw,0);cx.quadraticCurveTo(fw*.5,fh*.6,fw,fh*.5);cx.lineTo(0,fh*.5);cx.closePath();cx.fill();
    cx.fillStyle='#fff';cx.beginPath();cx.moveTo(0,fh*.5);cx.quadraticCurveTo(fw*.5,fh*.5,fw,fh*.5);cx.quadraticCurveTo(fw*.5,fh*1.05,fw,fh);cx.lineTo(0,fh);cx.closePath();cx.fill();
    cx.restore();
  }
  ...
  function drawWalker(x,y){
    ...
    // Legs with shoes
    cx.save();cx.translate(-4*u,-18*u);cx.rotate(wk);
    cx.fillStyle='#374151';cx.beginPath();cx.roundRect(-4*u,0,8*u,16*u,3*u);cx.fill();
    cx.fillStyle='#111';cx.beginPath();cx.roundRect(-4.5*u,14*u,9*u,5*u,2*u);cx.fill();
    cx.restore();
    cx.save();cx.translate(4*u,-18*u);cx.rotate(-wk);
    cx.fillStyle='#1F2937';cx.beginPath();cx.roundRect(-4*u,0,8*u,16*u,3*u);cx.fill();
    cx.fillStyle='#111';cx.beginPath();cx.roundRect(-4.5*u,14*u,9*u,5*u,2*u);cx.fill();
    cx.restore();
    // Shoes detail
    ...
    // Backpack
    cx.fillStyle='#059669';cx.beginPath();cx.roundRect(-13*u,-34*u,4*u,14*u,2*u);cx.fill();
    cx.fillStyle='#047857';cx.beginPath();cx.roundRect(9*u,-34*u,4*u,14*u,2*u);cx.fill();
    // Arms swinging
    cx.save();cx.translate(-11*u,-30*u);cx.rotate(-wk*.5);
    cx.fillStyle='#10B981';cx.beginPath();cx.roundRect(-3*u,0,7*u,13*u,3*u);cx.fill();
    cx.fillStyle=snap.cozy.skin;cx.beginPath();cx.roundRect(-3*u,11*u,7*u,5*u,2*u);cx.fill();
    cx.restore();
    cx.save();cx.translate(11*u,-30*u);cx.rotate(wk*.5);
    cx.fillStyle='#10B981';cx.beginPath();cx.roundRect(-3*u,0,7*u,13*u,3*u);cx.fill();
    cx.fillStyle=snap.cozy.skin;cx.beginPath();cx.roundRect(-3*u,11*u,7*u,5*u,2*u);cx.fill();
    ...
    // Hat
    cx.fillStyle='#F59E0B';cx.beginPath();cx.roundRect(-13*u,-62*u,26*u,8*u,4*u);cx.fill();
    cx.fillStyle='#D97706';cx.beginPath();cx.roundRect(-13*u,-58*u,26*u,3*u,1*u);cx.fill();
    // Hair
    ...
    cx.fillStyle='#111';cx.beginPath();cx.arc(-5*u,-51*u,1.2*u,0,Math.PI*2);cx.fill();
    cx.beginPath();cx.arc(5*u,-51*u,1.2*u,0,Math.PI*2);cx.fill();
    cx.fillStyle='rgba(255,255,255,.7)';cx.beginPath();cx.arc(-4*u,-52.5*u,.8*u,0,Math.PI*2);cx.fill();
    cx.beginPath();cx.arc(6*u,-52.5*u,.8*u,0,Math.PI*2);cx.fill();
    // Smile
    ...
    // Blush
    cx.fillStyle='rgba(220,100,80,.12)';cx.beginPath();cx.ellipse(-9*u,-48*u,3*u,2*u,0,0,Math.PI*2);cx.fill();
    cx.beginPath();cx.ellipse(9*u,-48*u,3*u,2*u,0,0,Math.PI*2);cx.fill();
    cx.restore();
  }
  ...
  function drawM2(){
    ...
    // Boundary markers
    const boundL=W*.03,boundR=W*.97;
    cx.strokeStyle='rgba(220,38,38,.6)';cx.lineWidth=3;cx.setLineDash([6,4]);
    cx.beginPath();cx.moveTo(boundL,gndY-2);cx.lineTo(boundL,gndY-H*.06);cx.stroke();
    cx.strokeStyle='rgba(30,64,175,.6)';cx.lineWidth=3;
    cx.beginPath();cx.moveTo(boundR,gndY-2);cx.lineTo(boundR,gndY-H*.06);cx.stroke();
    cx.setLineDash([]);
    cx.fillStyle='rgba(220,38,38,.5)';cx.font=`bold ${H*.018}px Bebas Neue`;cx.textAlign='center';
    cx.fillText('BATAS',boundL,gndY-H*.07);
    cx.fillStyle='rgba(30,64,175,.5)';cx.fillText('BATAS',boundR,gndY-H*.07);
    // Center marker on rope
    const ropeMid=(lx+rx)/2;
    cx.fillStyle='rgba(245,158,11,.8)';cx.beginPath();cx.arc(ropeMid,gndY-H*.2,5,0,Math.PI*2);cx.fill();
    cx.strokeStyle='#B45309';cx.lineWidth=2;
    cx.beginPath();cx.moveTo(ropeMid,gndY-H*.2-8);cx.lineTo(ropeMid,gndY-H*.2+8);cx.stroke();
    cx.beginPath();cx.moveTo(ropeMid-8,gndY-H*.2);cx.lineTo(ropeMid+8,gndY-H*.2);cx.stroke();
...
const App=(()=>{
  ...
  function ansM1(p,sel,correct,q){
    ...
    if(sel===correct){
      ...
      if(hq)hq[p+'ans']=sel;
      Snd.move();
      setTimeout(()=>{if(G[p])G[p].state='pull';Arena.setSnap({[p]:{score:ps.score,name:ps.name,state:'pull'}});checkBoundaryM1();},480);
    } else {
  ...
  function checkBoundaryM1(){
    ...
    // If rope center (midpoint) hits boundary
    const ropeMid=(x1+x2)/2;
    if(ropeMid<=boundL+10){endM1('p2','boundary');}
    else if(ropeMid>=boundR-10){endM1('p1','boundary');}
  }
  ...
  function ansM2(t,sel,correct,q){
    ...
    if(sel===correct){
      ...
      Snd.move();
      setTimeout(()=>{team.state='pull';Arena.setSnap({[tc]:{...team}});checkBoundaryM2();},480);
    } else {
      Snd.wrong();flashPanel(side,'wrong');
      if(hq){if(t===1)hq.t1ans=sel;else hq.t2ans=sel;}
  ...
  function checkBoundaryM2(){
    if(G.over)return;
    const n1=Math.max(1,G.t1.players.length),n2=Math.max(1,G.t2.players.length);
    const aW1=Math.min(W*.44,n1*W*.09),aW2=Math.min(W*.44,n2*W*.09);
    const sp1=aW1/n1,sp2=aW2/n2;
    const b1=W*.03,b2=W*.97-aW2;
    const lx=b1+n1*sp1+8,rx=b2-8;
    const ropeMid=(lx+rx)/2;
    const boundL=W*.03,boundR=W*.97;
    if(ropeMid<=boundL+10){endM2(2,'boundary');}
    else if(ropeMid>=boundR-10){endM2(1,'boundary');}
  }
  ...
  /* ═══════ BROWSER NOTE ═══════ */
  function closeBrowserNote(){const el=document.getElementById('browser-note');if(el)el.style.display='none';localStorage.setItem('kuisNoteSeen','1');}
  function showBrowserNote(){try{if(localStorage.getItem('kuisNoteSeen'))return;}catch(e){}const el=document.getElementById('browser-note');if(el)el.style.display='flex';}

  /* ═══════ INIT ═══════ */
  checkResume();showBrowserNote();
